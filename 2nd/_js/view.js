...
function updateLines() {
    ...
    for (var i = 0; i < hovered.length; i++) {
        ...
        if (element.getBoundingClientRect().left != 0) {
            ...
            linesContext.moveTo(
                element.getBoundingClientRect().left + document.documentElement.scrollLeft + element.clientWidth / 2, element.getBoundingClientRect().top + document.documentElement.scrollTop + 20
            );
            linesContext.lineTo(
                ~~(hovered[i].center[0] * zoom) + innerContainer.offsetLeft, ~~(hovered[i].center[1] * zoom) + innerContainer.offsetTop
            );
            linesContext.stroke();
...
function initView() {
    ...
    function highlightEntryFromUrl() {
        ...
        if (entry.length === 1) {
            ...
            scaleZoomOrigin = [
                1280 / 2 - entry.center[0] // + container.offsetLeft
                , 720 / 2 - entry.center[1] // + container.offsetTop
            ];
        ...
        for (var i = entriesOffset; i < entriesOffset + entriesLimit; i++) {
            ...
            element.addEventListener("mouseenter", function (e) {
                if (!fixed && !dragging) {
                    ...
                    scaleZoomOrigin = [
                        1280 / 2 - this.entry.center[0], 720 / 2 - this.entry.center[1]
                    ]
