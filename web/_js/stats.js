...
for (var i = 0; i <= Math.ceil((mostCounts / linesDistance) / 5) * 5; i++) {
	ctx.beginPath();
	ctx.moveTo(
		50, ~~(el.height - 50 - i * (linesDistance / mostCounts) * (el.height - 100)) + 0.5
	);
	ctx.lineTo(
		el.width - 25, ~~(el.height - 50 - i * (linesDistance / mostCounts) * (el.height - 100)) + 0.5
	);
	ctx.stroke();
}
...
for (var i = 0; i <= Math.ceil(mostCounts / linesDistance); i++) {
	...
	ctx.fillText(
		i * linesDistance, 40, ~~(el.height - 50 - i * (linesDistance / mostCounts) * (el.height - 100)) + 0.5
	);
}
...
for (var i = 0; i <= counts.length; i++) {
	if (i % skip == 0) {
		...
		ctx.beginPath();
		ctx.moveTo(
			~~(((i) / steps) * (el.width - 125) + 75) + 0.5, ~~(el.height - 50) + 0.5
		);
		ctx.lineTo(
			~~(((i) / steps) * (el.width - 125) + 75) + 0.5, y
		);
		ctx.stroke();

		ctx.fillText(
			(i) * (max / steps), ~~(((i) / steps) * (el.width - 125) + 75) - 0.5, y + 5
		);
	}
}
...
for (var i = 0; i < counts.length; i++) {
	...
	ctx.fillRect(
		~~((i / steps) * (el.width - 125) + 75), el.height - 50, Math.ceil(1 / steps * (el.width - 125)), ~~(-(counts[i] / mostCounts) * (el.height - 100))
	);
